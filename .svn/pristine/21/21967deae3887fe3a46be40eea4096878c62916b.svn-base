...
O objetivo principal desta tarefa era a implementação da função (@jogada@). Com a implementação
desta função é possivel atualizar o 'Estado' perante uma certa 'Jogada' feita.

= Objetivos

Sendo o objetivo principal, a implementação da (@jogada@), tivemos que separar o trabalho em tarefas
mais simples. Era necessário verificar se quando um 'Jogador' mudasse de pista ele embatia numa numa 
"parede". Isto é uma 'Peca' que se encontrava a uma altura superior, cuja diferença fosse maior que
0.2 unidades. Outra parte do trabalho foi implementar o resultado de um disparo de cola. Era necessário
alterar o 'Mapa' e a quantidade de munição. Por fim, implementou-se uma a função (@acelOuDesacel@) que 
simplesmente atualizava o 'aceleraJogador' para True ou para False dependendo do input recebido.

= Conclusão

Como resultado, obtivemos uma função que altera o 'Estado'. Foi um tarefa que requereu algum trabalho 
mais teórico e matemático, por exemplo no cálculo da altura da rampa numa dada posição. Mas, de forma
geral não foi muito elaborada. 
